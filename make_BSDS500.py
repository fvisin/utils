...
"""
Process the uncompressed BSDS500 dataset.
...
def main(path="./BSDS500",
         ...
         complib='blosc',
         chunksize=64):
    """This script will process BSDS500 images and create an hdf5 dataset.
    ...
    * x_std_dev: the standard deviation of each __pixel__
    ...
    impath = path + '/BSDS500/data/images/'
    gtpath = path + '/BSDS500/data/groundTruth/'
    train_dir = 'train'
    valid_dir = 'val'
    test_dir = 'test'
    ...
    img_list = []
    synsets = loadmat(metafile)['synsets']
    targets = dict([(synsets[i][0][1].item(), synsets[i][0][0].item() - 1)
                   for i in range(len(synsets))])
    assert min(targets.values()) == 0

    val_targets = []
    for directory, _, images in os.walk(impath):
        target = targets[directory]
        for img in images:
            impath = os.path.join(directory, img)
            img_list.append(impath)
            val_targets.append(target)


    #val_targets = numpy.asarray(map(int, open(gtpath).read().split())) - 1
    val_targets = []
    for directory, _, images in os.walk(impath):
        for img in images:
            impath = os.path.join(directory, img)
...
if __name__ == "__main__":
    main(path="/home/francesco/exp/datasets/BSDS500",
         randomize=False)
