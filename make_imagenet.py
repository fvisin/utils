...
def main(im_path="./val_images",
         ...
         complib='blosc',
         chunksize=64):
    """This script will process ImageNet images and create an hdf5 dataset.

    The images are resized to 256x256, center cropping them if necessary. The
    corresponding labels are read from either the ground truth files or the
    meta.mat file. Finally, pixel statistics are computed and saved. The
    output hdf5 file contains:
    * x: the resized and centered images, with shape [n_images, 256, 256, 3]
    * y: the classes, with shape [n_images, 1]. Note that the first class is 0.
    * x_mean: the mean value of each __pixel__
    * x_std_dev: the standard deviation of each __pixel__
    ...
    gt_file : string
        For the validation or the test set, the full path of the ground
        truth txt file.
    out_path : string
        The directory in which the output hdf5 file should be written.
    out_name : string
        The name of the output hdf5 file.
    is_training_set : bool
        Should be True if the set being processed is the training set.
    randomize : bool
        If true, the images will be shuffled before saving them to disk.
    complevel : int
        The compression level. If 0 compression is disabled.
    complib : string
        The compression library. Refer to `pytables` for details.
    chunksize : int
        The first dimension of the chunks. Ideally this should be a equal to
        the expected batch size or a multiple. The output hdf5 file will be
        created with chunk=[chunksize, 256, 256, 3]
    ...
    Note also that the first class is 0.
    """
...
if __name__ == "__main__":
    main(im_path="./train_images",
         meta_file="./meta.mat",
         out_path="/Tmp/imagenet",
         out_name="imagenet_2010_train",
         is_training_set=True,
         randomize=True)
    main(im_path="./val_images",
         gt_file="./ILSVRC2010_validation_ground_truth.txt",
         out_path="/Tmp/imagenet/",
         out_name="imagenet_2010_valid",
         is_training_set=False,
         randomize=False)
    main(im_path="./test_images",
         gt_file="./ILSVRC2010_test_ground_truth.txt",
         out_path="/Tmp/imagenet/",
         out_name="imagenet_2010_test",
         is_training_set=False,
         randomize=False)
